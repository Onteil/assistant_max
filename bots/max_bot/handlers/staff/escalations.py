...
def register_escalation_handlers(router):
    """Register all escalation handlers with the router."""
    from bots.max_bot.payloads import EscalationPayload, OperationsMenuPayload
    
    # List escalations (from operations menu)
    router.message_callback(
        OperationsMenuPayload.filter(lambda p: p.action == "escalations")
    )(handle_escalations_list)
    
    # List escalations (pagination)
    router.message_callback(
        EscalationPayload.filter(lambda p: p.action == "list")
    )(lambda event, payload, context, session, messenger_adapter: 
        handle_escalations_list(event, payload, context, session, messenger_adapter, page=payload.page or 0))
    
    # View escalation details
    router.message_callback(
        EscalationPayload.filter(lambda p: p.action == "view")
    )(handle_escalation_view)
    
    # Reassign escalation
    router.message_callback(
        EscalationPayload.filter(lambda p: p.action == "reassign")
    )(handle_escalation_reassign)
    
    # Confirm reassignment
    router.message_callback(
        EscalationPayload.filter(lambda p: p.action == "reassign_confirm")
    )(handle_escalation_reassign_confirm)
    
    # Take over escalation
    router.message_callback(
        EscalationPayload.filter(lambda p: p.action == "take_over")
    )(handle_escalation_take_over)
    
    # View staff contact
    router.message_callback(
        EscalationPayload.filter(lambda p: p.action == "contact")
