...
from bots.tg_bot.callback_datas import ProfileCallback, OrganizationCallback, KeyCallback
from bots.tg_bot.keyboards.profile_kb import get_profile_actions_keyboard, get_cancel_profile_action_keyboard
...
async def show_profile_menu(
    ...
    """
    Helper function to display user profile with action buttons.
    
    Used after profile operations to return user to profile view.
    """
    try:
        # Expire all cached objects to ensure fresh data from database
        # This is needed because expire_on_commit=False in session config
        session.expire_all()
        ...
        # Get organizations using user.id
        organizations = await get_user_organizations(session, user.id)
        
        # Get keys using user.id
        keys = await get_user_keys(session, user.id)
        
        # Limit display to first 10 organizations
        display_orgs = organizations[:10]
        has_more_orgs = len(organizations) > 10
        
        # Limit display to first 10 keys
        display_keys = keys[:10]
        has_more_keys = len(keys) > 10
        ...
        logger.error(
            f"Database error displaying profile menu for user {telegram_id}: {e}",
            exc_info=True
        )
        await message.answer(
...
async def show_organizations_list(
    ...
    """
    Display paginated list of user's organizations with delete buttons.
    ...
    try:
        ...
        if total_count > items_per_page:
            ...
            pagination_info = f"Страница {current_page} из {total_pages}"
        else:
            pagination_info = ""
        
        # Build message
        from bots.tg_bot.texts import PROFILE_ORGANIZATIONS_LIST
        text = PROFILE_ORGANIZATIONS_LIST.format(
            total_count=total_count,
            pagination_info=pagination_info
        )
        
        # Get keyboard with organizations as buttons
        from bots.tg_bot.keyboards.profile_kb import get_organizations_list_keyboard
        keyboard = await get_organizations_list_keyboard(organizations, page, items_per_page)
        ...
        logger.error(
            f"Database error showing organizations list for user {telegram_id}: {e}",
            exc_info=True
        )
        await callback.answer(
            "❌ Произошла ошибка при загрузке списка организаций",
            show_alert=True
        )
...
async def show_keys_list(
    ...
    """
    Display paginated list of user's GS keys with delete buttons.
    ...
    try:
        ...
        # Build message
        from bots.tg_bot.texts import PROFILE_KEYS_LIST
        text = PROFILE_KEYS_LIST.format(
            total_count=total_count,
            pagination_info=pagination_info
        )
        
        # Get keyboard with keys as buttons
        from bots.tg_bot.keyboards.profile_kb import get_keys_list_keyboard
        keyboard = await get_keys_list_keyboard(keys, page, items_per_page)
        ...
        logger.error(
            f"Database error showing keys list for user {telegram_id}: {e}",
            exc_info=True
        )
        await callback.answer(
            "❌ Произошла ошибка при загрузке списка ключей",
            show_alert=True
        )
...
async def show_profile(message: Message, session: AsyncSession):
    ...
    try:
        ...
        # Get organizations using user.id (not telegram_id)
        organizations = await get_user_organizations(session, user.id)
        
        # Get keys using user.id (not telegram_id)
        keys = await get_user_keys(session, user.id)
...
async def handle_profile_action(
    ...
    - cancel: Cancel current operation and return to profile
    ...
    try:
        if action == "cancel":
            ...
            if user:
                ...
                # Show profile with action buttons
                keyboard = await get_profile_actions_keyboard()
                await callback.message.edit_text(profile_text, reply_markup=keyboard)
            
            logger.info(f"User {telegram_id} cancelled profile operation")
            return
        
        elif action == "view_orgs":
            await show_organizations_list(callback, session, callback_data.page or 0)
        
        elif action == "view_keys":
            await show_keys_list(callback, session, callback_data.page or 0)
        
        elif action == "back":
            # Return to profile view
            await callback.answer()
            # Use show_profile_menu but adapt for callback
            telegram_id = callback.from_user.id
            ...
            if user:
                ...
                # Format subscription status
                if user.subscription_status and user.subscription_status.value == "active":
                    subscription_status = PROFILE_ACTIVE_SUBSCRIPTION.format(
                        expiry_date=user.subscription_end_date.strftime("%d.%m.%Y")
                    ) if user.subscription_end_date else "✅ Активна"
                elif user.subscription_status and user.subscription_status.value == "expired":
                    subscription_status = PROFILE_EXPIRED_SUBSCRIPTION.format(
                        expiry_date=user.subscription_end_date.strftime("%d.%m.%Y")
                    ) if user.subscription_end_date else "⚠️ Истекла"
                else:
                    subscription_status = PROFILE_NO_SUBSCRIPTION
                
                notification_status = PROFILE_NOTIFICATIONS_ON if user.notification_preferences else PROFILE_NOTIFICATIONS_OFF
                reg_date = user.created_at.strftime("%d.%m.%Y") if user.created_at else "Неизвестно"
            ...
            # Check if we're already in notifications menu (toggling) or opening it first time
            # If message already contains notifications menu text, toggle
            # Otherwise, show the menu first
            if "Управление уведомлениями" in callback.message.text:
                await toggle_notifications(callback, session, telegram_id)
            else:
                await show_notifications_menu(callback, session, telegram_id)
...
async def add_organization(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = await get_cancel_profile_action_keyboard()
    await callback.message.edit_text(PROFILE_ADD_INN_PROMPT, reply_markup=keyboard)
    await callback.answer()
...
async def process_new_inn(
    ...
    try:
        ...
        existing_orgs = await get_user_organizations(session, user.id)
        ...
        organization = await add_user_organization(session, user.id, inn)
        ...
        # Show profile menu again (session will query fresh data after commit)
        await show_profile_menu(message, session, telegram_id)
        ...
        # Show profile menu again (session will query fresh data)
        await show_profile_menu(message, session, telegram_id)
        ...
        # Show profile menu again even on error (session will query fresh data)
        await show_profile_menu(message, session, telegram_id)
...
async def add_gs_key(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = await get_cancel_profile_action_keyboard()
    await callback.message.edit_text(PROFILE_ADD_KEY_PROMPT, reply_markup=keyboard)
    await callback.answer()
...
async def process_new_key(
    ...
    try:
        ...
        logger.error(
            f"Database error getting user for key addition {telegram_id}: {e}",
            exc_info=True
        )
        await message.answer(
    ...
    try:
        conflict_response = await api_client.check_key_conflict(
            ...
            user_id=user.id  # Use user.id instead of telegram_id
        )
    ...
    try:
        ...
        gs_key = await add_user_key(
            session,
            user.id,  # Use user.id instead of telegram_id
            normalized_key,
            conflict_status
        )
        ...
        if conflict_detected:
            await message.answer(ADD_KEY_CONFLICT)
        else:
            await message.answer(ADD_KEY_SUCCESS.format(key_number=normalized_key))
        ...
        await message.answer("⚠️ Этот ключ уже добавлен в ваш профиль.")
...
async def create_key_conflict_ticket(
    ...
    try:
        ticket = Ticket(
            ...
            user_id=user_id,  # Use user_id instead of tg_user_id
            description=(
        ...
        logger.error(
            f"Error creating key conflict ticket for user_id {user_id}: {e}",
            exc_info=True
        )
...
async def request_phone_change(callback: CallbackQuery, state: FSMContext):
    ...
    keyboard = await get_cancel_profile_action_keyboard()
    await callback.message.edit_text(PROFILE_CHANGE_PHONE_PROMPT, reply_markup=keyboard)
    await callback.answer()
...
async def process_phone_change_request(
    ...
    try:
        ...
        ticket = Ticket(
            ticket_type=TicketType.RENEWAL,  # Using RENEWAL type for admin tasks
            ticket_status=TicketStatus.NEW,
            user_id=user.id,  # Use user.id instead of tg_user_id
            description=(
        ...
        # Show profile menu again even on error
        await show_profile_menu(message, session, telegram_id)
...
async def show_notifications_menu(
    ...
    Displays current notification status and toggle button.
    ...
    try:
        ...
        if not user:
            await callback.answer("❌ Пользователь не найден", show_alert=True)
            logger.warning(f"Show notifications menu for non-existent user: {telegram_id}")
            return
        ...
        from bots.tg_bot.keyboards.profile_kb import get_notifications_menu_keyboard
        keyboard = await get_notifications_menu_keyboard(user.notification_preferences)
        ...
        logger.info(f"Notifications menu shown for user {telegram_id}")
    
    except SQLAlchemyError as e:
        logger.error(
            f"Database error showing notifications menu for user {telegram_id}: {e}",
            exc_info=True
        )
        await callback.answer(
...
async def toggle_notifications(
    ...
    """
    Toggle notification preferences and show notifications menu.
    ...
    try:
        ...
        # Show notifications menu with toggle button
        from bots.tg_bot.keyboards.profile_kb import get_notifications_menu_keyboard
        keyboard = await get_notifications_menu_keyboard(user.notification_preferences)
...
# ========== Delete Organization/Key Handlers ==========


@router.callback_query(OrganizationCallback.filter(F.action == "delete"))
async def delete_organization(
    ...
    """
    Handle organization deletion request.
    
    Args:
        callback: Callback query from delete button
        callback_data: Contains INN to delete
        session: Database session
    """
    telegram_id = callback.from_user.id
    ...
    if not inn:
        await callback.answer("❌ Ошибка: ИНН не указан", show_alert=True)
        return
    ...
    try:
        ...
        from database.models import user_organizations
        ...
        if result.rowcount > 0:
            await callback.answer(f"✅ Организация {inn} удалена")
            logger.info(f"User {telegram_id} deleted organization {inn}")
        else:
            await callback.answer("⚠️ Организация не найдена", show_alert=True)
        
        # Refresh the organizations list
        page = callback_data.page or 0
        await show_organizations_list(callback, session, page)
        
    except SQLAlchemyError as e:
        await session.rollback()
        logger.error(
            f"Database error deleting organization for user {telegram_id}: {e}",
            exc_info=True
        )
        await callback.answer(
...
@router.callback_query(KeyCallback.filter(F.action == "delete"))
async def delete_key(
    ...
    """
    Handle GS key deletion request.
    
    Args:
        callback: Callback query from delete button
        callback_data: Contains key_id to delete
        session: Database session
    """
    telegram_id = callback.from_user.id
    ...
    if not key_id:
        await callback.answer("❌ Ошибка: ID ключа не указан", show_alert=True)
        return
    ...
    try:
        ...
        if result.rowcount > 0:
            ...
            logger.info(f"User {telegram_id} deleted key {key_id}")
        else:
            await callback.answer("⚠️ Ключ не найден", show_alert=True)
        
        # Refresh the keys list
        page = callback_data.page or 0
        await show_keys_list(callback, session, page)
        
    except SQLAlchemyError as e:
        await session.rollback()
        logger.error(
            f"Database error deleting key for user {telegram_id}: {e}",
            exc_info=True
        )
        await callback.answer(
