...
### Диагностика очереди i-TAT

`api_retry_queue` сохраняет временно неудачные вызовы i-TAT независимо от уведомлений в MAX. На prod используйте:

```dotenv
ENABLE_API_RETRY_DIAGNOSTICS="False"
```

При `true` администраторам MAX приходят временные подробные уведомления о новых записях в очереди, включая payload. Не включайте этот режим в рабочем чате. При исчерпании всех попыток остаётся обычное операционное уведомление, но без payload.
